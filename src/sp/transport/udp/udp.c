...
typedef enum udp_disc_reason {
    ...
    DISC_NEGO = 5,     // negotiation failed
...
static void udp_ep_cleanup_closed_pipes(udp_ep *ep, const nng_sockaddr *sa);
static nng_err udp_ep_redial(udp_ep *ep);
...
udp_pipe_fini(void *arg)
{
    ...
    // NOTE: This may be called without ep->mtx held (from pipe_destroy
    // via nni_pipe_rele). This is safe because the pipe has already been
    // removed from ep->pipes and no concurrent access to rx_mq or
    // rx_aios is possible at this point.
...
udp_find_pipe(udp_ep *ep, const nng_sockaddr *peer_addr)
{
    ...
    for (;;) {
        ...
        if (nng_sockaddr_equal(&p->peer_addr, peer_addr) && !p->closed) {
            return (p);
        }
        id++;
        if (id == 0) {
...
static void
udp_queue_tx(udp_ep *ep, const nng_sockaddr *sa, udp_sp_msg *msg, nni_msg *payload)
{
    udp_txring * ring = &ep->tx_ring;
...
udp_send_disc(udp_ep *ep, udp_pipe *p, udp_disc_reason reason)
{
    ...
    // NOTE: Caller is responsible for calling nni_pipe_close(p->npipe)
    // to trigger pipe_reap and NNG_PIPE_EV_REM_POST callback.
}
...
udp_recv_disc(udp_ep *ep, udp_sp_msg *disc, const nng_sockaddr *sa)
{
    ...
    if (p != NULL) {
        ...
        // Remove from connpipes list if present
        nni_list_node_remove(&p->node);
        // Remove from pipes map immediately so udp_timer_cb
        // cannot find and re-process this pipe
        udp_remove_pipe(p);
        // Defer npipe close to avoid deadlock: ep->mtx is held by
        // udp_rx_cb, but nni_pipe_close -> pipe_reap -> udp_pipe_close
        // needs to acquire ep->mtx.
        if (p->npipe != NULL) {
            nni_list_append(&ep->defer_close_list, p);
        }
    }
}

// Receive data for the pipe.
static void
udp_recv_data(udp_ep *ep, udp_sp_msg *dreq, size_t len, const nng_sockaddr *sa)
{
    ...
    nni_msg_chop(ep->rx_payload, nni_msg_len(ep->rx_payload) - dreq->us_length);
    ...
    while (((aio = nni_list_first(&p->rx_aios)) != NULL) && (!nni_lmq_empty(&p->rx_mq))) {
        nni_aio_list_remove(aio);
        nni_lmq_get(&p->rx_mq, &msg);
        nni_aio_set_msg(aio, msg);
        nni_aio_completions_add(&ep->complq, aio, 0, nni_aio_count(aio));
    }
}
...
udp_rx_cb(void *arg)
{
    ...
    // Dialers cannot receive connection requests (as a safety precaution).
...
udp_ep_stop(void *arg)
{
    ...
    nni_time linger = nni_clock() + NNI_SECOND / 2; // half second to drain, max
    nni_mtx_lock(&ep->mtx);
...
udp_timer_cb(void *arg)
{
    ...
    // Phase 1: collect pipes that need to be reaped (closed or expired).
    // We use a linked list to avoid modifying the id_map during iteration.
    nni_list reap_list;
    NNI_LIST_INIT(&reap_list, udp_pipe, node);

    while (nni_id_visit(&ep->pipes, NULL, (void **)&p, &cursor)) {
        if (p->closed) {
            // Already closed (e.g., by udp_recv_disc or udp_send_disc).
            // Collect for reaping after iteration.
            nni_list_node_remove(&p->node);
        ...
        if (now > p->expire) {
            ...
            // Possibly alert the dialer, so it can restart a new attempt.
        ...
        // Send CREQ if needed (dialer only)
...
static int
udp_ep_init(udp_ep *ep, nng_url *url, nni_sock *sock, nni_dialer *d, nni_listener *l)
{
    int rv;
    ...
    ep->tx_ring.descs = NNI_ALLOC_STRUCTS(ep->tx_ring.descs, NNG_UDP_TXQUEUE_LEN);
    ...
    nni_stat_init_lock(&ep->st_peer_inactive, &peer_inactive_info, &ep->mtx);
...
udp_resolv_cb(void *arg)
{
    ...
    if ((rv = nni_pipe_alloc_dialer((void **)&p, ep->ndialer)) != NNG_OK) {
        nni_aio_list_remove(aio);
        nni_mtx_unlock(&ep->mtx);
        nni_aio_finish_error(aio, rv);
        return;
...
// udp_ep_cleanup_closed_pipes removes all closed pipes for a given peer
// address from ep->pipes. This must be called with ep->mtx held, and
// ensures that stale pipes from a previous connection do not interfere
// with redial (e.g. by occupying the hash slot that a new pipe would
// need, or by being found by udp_find_pipe before the new pipe).
static void
udp_ep_cleanup_closed_pipes(udp_ep *ep, const nng_sockaddr *sa)
{
    uint64_t id = nng_sockaddr_hash(sa);
    for (;;) {
        udp_pipe *p;
        p = nni_id_get(&ep->pipes, id);
        if (p == NULL) {
            // No more entries at this key or its probes.
            return;
        }
        if (nng_sockaddr_equal(&p->peer_addr, sa) && p->closed) {
            // Use udp_remove_pipe which handles list node
            // cleanup and reference counting properly.
            // After removal, the current id slot is free;
            // restart from the original hash to catch any
            // remaining closed pipes that may have been
            // at higher probe positions.
            udp_remove_pipe(p);
            id = nng_sockaddr_hash(sa);
            continue;
        }
        id++;
        if (id == 0) {
...
// udp_ep_redial creates a new pipe and sends a CREQ for reconnection,
// skipping DNS resolution and UDP socket creation since they are cached.
// Caller must hold ep->mtx. Returns 0 on success (aio remains pending
// in connaios), or an error code (caller must remove aio from connaios,
// release ep->mtx, close the pipe if allocated, and finish the aio).
static nng_err
udp_ep_redial(udp_ep *ep)
{
    udp_pipe *p;
    nng_err   rv;
...
udp_ep_connect(void *arg, nni_aio *aio)
{
    ...
    if (ep->started) {
        // Reconnection: if we have a cached UDP socket and peer
        // address (from a previous successful connection), and we
        // are a dialer, allow redialing directly.
        if (ep->udp != NULL && ep->dialer) {
            int rv;
...
static nng_err
udp_dialer_setopt(void *arg, const char *name, const void *buf, size_t sz, nni_type t)
{
    udp_ep *ep = arg;
...
static nng_err
udp_listener_setopt(void *arg, const char *name, const void *buf, size_t sz, nni_type t)
{
    udp_ep *ep = arg;
